    ...
    */
    public class LevenbergMarquardt : OptimizationMethod
    {
        ...
        public override EndCriteria.Type minimize(Problem P, EndCriteria endCriteria)
        {
            ...
            List<int> ipvt = new daLib.Patterns.InitializedList<int>(n);
            ...
            if (n <= 0)
            {
                throw new ExcelException("no variables given");
            }

            if (m < n)
            {
                throw new ExcelException($"less functions ({m}) than available variables ({n})");
            }

            if (endCriteria.functionEpsilon() < 0.0)
            {
                throw new ExcelException("negative f tolerance");
            }

            if (xtol_ < 0.0)
            {
                throw new ExcelException("negative x tolerance");
            }

            if (gtol_ < 0.0)
            {
                throw new ExcelException("negative g tolerance");
            }

            if (endCriteria.maxIterations() <= 0)
            {
                throw new ExcelException("null number of evaluations");
            }
            ...
            if (info == 0)
            {
                throw new ExcelException("MINPACK: improper input parameters");
            }
            ...
            if (info == 7)
            {
                throw new ExcelException("MINPACK: xtol is too small. no further " +
            ...
            if (info == 8)
            {
                throw new ExcelException("MINPACK: gtol is too small. fvec is " +
